...
(() => {
 ...
 root.innerHTML=`<div class="cal-topline"><span class="cal-label">${tr('Your next collaboration','งานร่วมกันครั้งถัดไป')}</span><span class="cal-demo">${tr('Interactive demo','ตัวอย่างทดลองใช้')}</span></div>
 <div class="cal-shell">
 <aside class="cal-panel cal-details"><img class="cal-avatar" src="image/about/01.webp" alt=""><div class="cal-owner"></div><h3>${tr('Booking request','ส่งคำขอจองคิว')}</h3><p class="cal-muted">${tr('A new story starts here. Choose a date and time for your next shoot or creative collaboration.','เรื่องราวใหม่เริ่มต้นที่นี่ เลือกวันและเวลาสำหรับงานถ่ายภาพหรือโปรเจกต์สร้างสรรค์ครั้งถัดไป')}</p>
 ...
 <form class="cal-panel cal-form" hidden><h3>${tr('A few details.','รายละเอียดเพิ่มเติม')}</h3><p class="cal-summary"></p><p class="cal-muted">${tr('Preview only. Use sample details; nothing is sent or saved.','ตัวอย่างเท่านั้น ใช้ข้อมูลสมมติ ไม่มีการส่งหรือบันทึกข้อมูล')}</p><div class="cal-fields"><label>${tr('Name or brand','ชื่อหรือแบรนด์')}<input name="name" required maxlength="80" autocomplete="off" placeholder="Alex / Studio A"></label><label>${tr('Email','อีเมล')}<input name="email" type="email" required autocomplete="off" placeholder="alex@example.com"></label><label>${tr('Project type','ประเภทงาน')}<select name="project"><option>${trText('Editorial','เอดิทอเรียล')}</option><option>Campaign</option><option>Creative direction</option></select></label><label>${tr('Location','สถานที่')}<input name="location" placeholder="Bangkok" autocomplete="off"></label><label class="cal-wide">${tr('Tell me about your project','รายละเอียดโปรเจกต์')}<textarea name="notes" placeholder="A little about the concept, team, and timing…"></textarea></label></div><div class="cal-form-actions"><button type="button" data-back>${tr('← Back','← กลับ')}</button><button class="cal-primary" type="submit">${tr('Preview request →','ดูตัวอย่างคำขอ →')}</button></div></form>
 <section class="cal-success" hidden aria-live="polite"><div class="cal-success-mark">✓</div><h3 tabindex="-1">${tr('Your next chapter.','เรื่องราวบทต่อไปของคุณ')}</h3><p>${tr('This is how the request confirmation could look.','ตัวอย่างหน้าการยืนยันคำขอจองคิว')}</p><p class="cal-summary"></p><p class="cal-muted">${tr('Demo complete. No booking was made and no details were sent.','จบการทดลอง ไม่มีการจองคิวหรือส่งข้อมูล')}</p><button type="button" data-reset>${tr('Explore another date','ลองเลือกวันอื่น')}</button></section></div>
 <div class="cal-footnote"><span>${tr('Cal.com-style preview · illustrative availability','ตัวอย่างรูปแบบ Cal.com · เวลาว่างสมมติ')}</span><span>${tr('Final live appearance depends on your Cal.com setup.','หน้าตาจริงขึ้นอยู่กับการตั้งค่า Cal.com ของคุณ')}</span></div>`;
